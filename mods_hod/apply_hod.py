...
from .hod_models import MWCens, MWSats, ContrerasCens, ContrerasSats, HMQCens
...
class ModelClass():
    ...
    def compute_model_instance(self):
        ...
        if self.model == 0:
            ...
            cens_occ_model = HMQCens(redshift=self.redshift)
            sats_occ_model = MWSats(redshift=self.redshift) # with the same satellite occupation as the MW's
